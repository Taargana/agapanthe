...
>
> ### ▶️ Reprise — Contenu-3 (prefabs & scènes déclaratifs), ou 2ᵉ slice dissemblable, ou UI-3
> **Contenu-2 est CLOS.** Feu vert humain requis avant **Contenu-3** — `.agscene` / `.agprefab` (format
> d'**authoring**, PAS une sauvegarde ; ⚠️ le registry `ISceneRecipe` actuel est de l'*organisation de code*)
> + `SceneLoader` + les 5 recipes Sandbox → données. **C'est là que se prend la décision structurante
> « identité d'asset côté simulation »** (composant `AssetRef` — finding archi Contenu-1 ; `MeshRef` render-local
> ⇒ un `Save` headless écrit `AssetKey.None` ⇒ un serveur autoritaire ne peut pas émettre d'état visuellement
> reconstructible). Alternatives : **2ᵉ slice dissemblable** (top-down — fera mordre la scission `SceneContext`) ·
> **UI-3** (timestamps GPU + seam `FrameProfiler`).
>
