...
> atomique + throw nommé au lieu d'un overwrite silencieux ; +9 tests). **Verdict visuel humain : DÛ** (partagé avec S30).
>
...
>
> ### ▶️ Reprise — Contenu-2 (cook offline + manifest), ou 2ᵉ slice dissemblable, ou UI-3
> **Contenu-1 est CLOS.** Feu vert humain requis avant **Contenu-2** — `tools/AssetCooker` (patron `FontCooker`), blobs
> binaires déterministes keyés par `AssetKey`, import glTF → offline, manifest `clé → blob + deps`. À cadrer en interview :
> `AssetKey` devient-elle aussi la clé du manifest ; `AssetKey.FromContentPath(root, path)`. Puis **Contenu-3** (prefabs
> & scènes déclaratifs — ⚠️ le registry `ISceneRecipe` actuel est de l'*organisation de code*, PAS un format d'authoring ;
> c'est là que se prend la décision « identité d'asset dans la simulation »). Alternatives : **2ᵉ slice dissemblable**
> (top-down — fera mordre la scission `SceneContext`) · **UI-3** (timestamps GPU + seam `FrameProfiler`).
>
