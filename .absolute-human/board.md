...
### M4-04 — Parser glTF JSON + conteneur GLB [code, M] — pending
System.Text.Json (source-gen ou JsonDocument — choisir ; TreatWarningsAsErrors + trimming pas un souci phase 1) : structures brutes du schéma (asset, scenes, nodes, meshes, primitives, accessors, bufferViews, buffers, materials, textures, images, samplers). GLB : header magic/version/length, chunks JSON+BIN. Résolution des URIs buffer (fichier relatif, data: base64). Erreurs : GraphicsException-équivalent Assets (`AssetException`) avec chemin+raison (spec §4). Tests : Box.gltf parse, GLB header/chunks, base64. AC: tests verts.

### M4-05 — Lecture des accessors → géométrie [code, M] — pending
Extraction typée : VEC3 float positions/normals, VEC4 float tangents, VEC2 float uv, indices SCALAR u16/u32 (→ u32 DTO). Gère byteStride (entrelacé) et bufferView offsets. Composants non-float (normalized u8/u16) : hors périmètre → AssetException claire. Validation min : counts cohérents, POSITION requis. Tests : Box (non entrelacé) + BoxInterleaved (entrelacé) → mêmes 24 sommets/36 indices attendus, valeurs de référence. AC: tests verts.
...
### M4-08 — ImageLoader StbImageSharp [code, S] — pending — OWNER Assets/ImageLoader.cs
Décode PNG/JPEG → RGBA8 bytes + dimensions (force 4 canaux). Usage sRGB/linéaire = paramètre appelant (décidé par le slot matériau, M4-06). data: URIs et fichiers. Tests : décode la texture de BoxTextured (dimensions attendues). AC: tests verts.
