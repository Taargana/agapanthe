...
**Prochain (session 23+)** : **Vertical Slice** (backlog §4ter — preuve d'intégration, ancre planétaire, Windows d'abord). **VS-1 sérialisation CLOSE** (S22). Suivant : **VS-2 spawn runtime** (`SpawnBodyDeferred` + `CommandKind.SpawnBody`, dette P3-M3 : `SpawnBody` est immédiat → besoin d'un spawn de corps différé pour la simulation en cours), puis VS-3 glu gameplay, VS-4 HUD, VS-5 audio (stretch). **P3-M0 validation Linux/macOS** reste dûe (non bloquante). Détail + point de reprise dans `docs/AVANCEMENT.md` § Reprise.
