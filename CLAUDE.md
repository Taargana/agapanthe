...
**Prochain (session 24+)** : **Vertical Slice** (backlog §4ter — preuve d'intégration, ancre planétaire, Windows d'abord). **VS-1 sérialisation CLOSE** (S22), **VS-2 spawn runtime + gravité newtonienne CLOSE** (S23). Suivant : **VS-3 glu gameplay** (assembler spawn+physique+sérialisation en une boucle jouable minimale), puis VS-4 HUD, VS-5 audio (stretch). **P3-M0 validation Linux/macOS** reste dûe (non bloquante). Détail + point de reprise dans `docs/AVANCEMENT.md` § Reprise.
