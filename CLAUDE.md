...
**Prochain (session 25+)** : **Vertical Slice** (backlog §4ter — preuve d'intégration, ancre planétaire, Windows d'abord). **VS-1 sérialisation CLOSE** (S22), **VS-2 spawn runtime + gravité newtonienne CLOSE** (S23), **VS-3 glu gameplay CLOSE** (S24 — spawn+physique+sérialisation assemblés en une boucle jouable). Suivant : **VS-4 HUD minimal** (overlay texte in-view — premier nouveau chemin de rendu depuis P3-M8 ; aujourd'hui le défi ne parle que par `window.Title`), puis VS-5 audio (stretch). **P3-M0 validation Linux/macOS** reste dûe (non bloquante). Détail + point de reprise dans `docs/AVANCEMENT.md` § Reprise.
